...
#
# Idempotent:
#   - If <email> already exists in profiles with base_role='super_admin' → write
#     a 'bootstrap_super_admin_replay' audit row and exit 0.
#   - If <email> exists with a different role → fail loudly. Don't auto-promote.
#   - Otherwise → create auth.users + profiles + send magic link + audit row.
#
# Required env:
...
if [[ "$count" -gt 0 ]]; then
  ...
  log "${EMAIL} already provisioned as super_admin; writing replay audit row"
  write_audit "$user_id" "bootstrap_super_admin_replay" "$user_id"
  log "done (replay)"
  exit 0
fi

# 2. Create auth.users row (no password; magic-link flow).
log "creating auth user for ${EMAIL}"
auth_user=$(curl -fsS -X POST "${AUTH}/admin/users" "${H_KEY[@]}" "${H_JSON[@]}" -d "$(jq -n --arg email "$EMAIL" '{email:$email, email_confirm:true}')")
...
curl -fsS -X POST "${AUTH}/admin/generate_link" "${H_KEY[@]}" "${H_JSON[@]}" -d "$(jq -n --arg email "$EMAIL" '{type:"magiclink", email:$email}')" > /dev/null

# 5. Audit row.
write_audit "$user_id" "bootstrap_super_admin" "$user_id"

log "✓ super_admin bootstrapped for ${EMAIL}; magic link sent"
