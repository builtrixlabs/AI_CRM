...
--
-- Pattern: every operational read/write is scoped by auth.org_id().
...
CREATE POLICY organizations_select_self ON organizations
  ...
  USING (id = auth.org_id() AND deleted_at IS NULL);
...
CREATE POLICY workspaces_select_org ON workspaces
  FOR SELECT TO authenticated
  USING (organization_id = auth.org_id() AND deleted_at IS NULL);
