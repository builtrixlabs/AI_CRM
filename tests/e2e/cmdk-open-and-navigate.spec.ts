/**
 * D-008 — Cmd+K open + navigate (@smoke).
 * Spec AC-1..AC-10: hotkey opens palette; type narrows; Enter navigates;
 * Esc closes.
 */
...
const ORG_SLUG = `e2e-cmdk-${Date.now()}`;
const REP_EMAIL = `e2e-cmdk-${Date.now()}@test.builtrix.in`;
const PASS = "T3st-pass-cmdk-nav!!!";
...
test.beforeAll(async () => {
  test.skip(
    !admin || !ANON_KEY,
    "Cmd+K e2e needs SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY + SUPABASE_PUBLISHABLE_KEY",
  );
  if (!admin) return;
...
test.describe("@smoke Cmd+K open + navigate", () => {
  test("opens with Ctrl+K, navigates on selection, closes on Esc", async ({
    page,
  }) => {
    await signIn(page, REP_EMAIL);
    await page.goto("/dashboard");
    // Use Ctrl+K (cross-platform; macOS browsers also accept).
    await page.keyboard.press("Control+KeyK");
    await expect(page.getByTestId("command-palette")).toBeVisible();
    const input = page.getByTestId("command-palette-input");
    await input.fill("demo lead");
    await page.keyboard.press("Enter");
    await page.waitForURL(/\/dashboard\/leads\/demo$/, { timeout: 10_000 });
